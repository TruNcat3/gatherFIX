...
@pytest.mark.parametrize(
    "make_index",
    [
        pytest.param("expand", id="expand"),  # issue #5746: stride like (1,0,0)
        pytest.param("transpose", id="transpose"),  # swapped strides
...
def test_gather_non_contiguous_index(dim, make_index):
    ...
    B, S, H = 4, 16, 64
    inp = torch.randn(B, S, H, dtype=torch.float16, device=flag_gems.device)
    size_dim = (B, S, H)[dim]

    if make_index == "expand":
        base_shape = list(inp.shape)
        base_shape[dim] = 1
        idx = torch.randint(
            0, size_dim, tuple(base_shape), dtype=torch.int64, device=flag_gems.device
        ).expand(inp.shape)
    elif make_index == "transpose":
        # source sized so that after permute+slice idx.shape[i] <= inp.shape[i]
        # holds for every i != dim (gather semantic constraint)
        src = torch.randint(
            0, min(inp.shape), (H, B, S), dtype=torch.int64, device=flag_gems.device
        )
        perm = [1, 0, 2] if dim != 1 else [2, 1, 0]
        idx = src.permute(*perm)
        idx = idx[: inp.shape[0], : inp.shape[1], : inp.shape[2]]
    else:  # slice: stride (2*H, H, 1), same shape as inp
        full = torch.randint(
            0, size_dim, (B, 2 * S, H), dtype=torch.int64, device=flag_gems.device
        )
        idx = full[:, ::2, :]

    ref_inp = utils.to_reference(inp)
    ref_idx = utils.to_reference(idx)
    ref_out = torch.gather(ref_inp, dim, ref_idx)

    with flag_gems.use_gems():
        res_out = torch.gather(inp, dim, idx)
